...
namespace DotNetty.Codecs.MqttFx
{
    ...
    public sealed class MqttDecoder2 : ReplayingDecoder<MqttDecoder2.ParseState>
    {
        ...
        public MqttDecoder2(bool isServer, int maxMessageSize)
            : base(ParseState.Ready)
        {
        ...
        Packet DecodePacketInternal(IByteBuffer buffer, int packetSignature, ref int remainingLength, IChannelHandlerContext context)
        {
            ...
            {
                var qualityOfService = (MqttQos)((packetSignature >> 1) & 0x3); // take bits #1 and #2 ONLY and convert them into QoS value
                //if (qualityOfService == MqttQos.Reserved)
                //{
        ...
        static void DecodeConnectPacket(IByteBuffer buffer, ConnectPacket packet, ref int remainingLength, IChannelHandlerContext context)
        {
            ...
            {
                var connAckPacket = new ConnAckPacket();
                connAckPacket.ConnectReturnCode = ConnectReturnCode.UnacceptableProtocolVersion;
            ...
            if (hasWill)
            {
                ...
                packet.WillQos = (MqttQos)((connectFlags & 0x18) >> 3);
                //if (packet.WillQos == MqttQos.Reserved)
                //{
                //    throw new DecoderException($"[MQTT-3.1.2-14] Unexpected Will QoS value of {(int)packet.Qos}.");
                //}
                packet.WillTopic = string.Empty;
            }
            ...
            packet.UsernameFlag = (connectFlags & 0x80) == 0x80;
            packet.PasswordFlag = (connectFlags & 0x40) == 0x40;
            if (packet.PasswordFlag && !packet.PasswordFlag)
            {
            ...
            packet.KeepAlive = DecodeUnsignedShort(buffer, ref remainingLength);
            ...
            if (hasWill)
            {
                packet.WillTopic = DecodeString(buffer, ref remainingLength);
                ...
                buffer.ReadBytes(packet.WillMessage, 0, willMessageLength);
            }
        ...
        static void DecodeConnAckPacket(IByteBuffer buffer, ConnAckPacket packet, ref int remainingLength)
        {
            ...
            packet.ConnectReturnCode = (ConnectReturnCode)(ackData & 0xFF);
        }
        ...
        static void DecodePublishPacket(IByteBuffer buffer, PublishPacket packet, ref int remainingLength)
        {
            ...
            if (remainingLength > 0)
            {
                ...
                payload.Retain();
                packet.Payload = new byte[remainingLength];
                payload.ReadBytes(packet.Payload);
                remainingLength = 0;
            }
            else
            {
        ...
        static ushort DecodeUnsignedShort(IByteBuffer buffer, ref int remainingLength)
        {
